...
void InternalConnection::receiveMessage()
{
	...
	if(!document.isNull() && (jsonError.error == QJsonParseError::NoError) && document.isObject())
	{
		...
		{
			QJsonValue value = object.value("direction");
			if(value.isDouble())
			{
				double direction = value.toVariant().toDouble();
				emit changeDirection(direction);
			}
		}
		if(object.contains("radar"))
		{
			QJsonValue value = object.value("radar");
			if(value.isBool())
			{
				bool radar = value.toVariant().toBool();
				emit changeRadarState(radar);
			}
		}
	}
	else
		cout << "Serial state ananlysis failed: " << string(message.data()) << endl;
}
...
void InternalConnection::establishInternalConnection(quint16 port)
{
	...
	if(m_innerConnection->waitForConnected(-1))
	{
		...
		connectionMessage.insert("serial", true);
...
void InternalConnection::getConfig(quint16 &port)
{
	...
	cout << "Serial internal config setted" << endl;
}
